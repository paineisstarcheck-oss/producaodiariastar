# -*- coding: utf-8 -*-
# ------------------------------------------------------------
# Painel de Produção por Vistoriador (Streamlit) - MULTI-MESES (STARCHECK)
# - Lê automaticamente os arquivos listados na planilha-índice (secrets.starcheck_index_sheet_id)
# - Consolida dados de todos os meses e lê METAS por mês
# - KPIs, Resumo (filtro FIXO/MÓVEL só aqui), Gráficos, Auditoria,
#   Rankings Mensal e do Dia
#
# AJUSTE (mesmo padrão do Tokyo corrigido):
# - Índice NÃO é mais "silencioso": mostra spinner e o erro real
# - Aceita colunas URL/LINK/PLANILHA/ARQUIVO e ATIVO/ATIVA/STATUS
# - Cache leve (índice + leitura de planilhas) para não recarregar tudo a cada interação
# - Se algumas planilhas falharem, mostra quais falharam (sem travar se houver dados suficientes)
# ------------------------------------------------------------
...
st.title("Produção por Vistoriador - STARCHECK")

# === Planilha-Índice (ARQUIVOS) ===
INDEX_SHEET_ID = (st.secrets.get("starcheck_index_sheet_id", "") or "").strip()
INDEX_TAB_NAME = "ARQUIVOS"
if not INDEX_SHEET_ID:
    st.error("Defina no secrets.toml a chave starcheck_index_sheet_id com o ID da planilha-índice da STARCHECK.")
    st.stop()
...
# =========================
# Cache: leitura de planilha (por sheet_id)
# =========================
...
# =========================
# Índice: leitura com erro real + tolerância a colunas
# =========================
def _yes(v) -> bool:
...
def load_ids_from_index_cached(index_sheet_id: str, tab_name: str) -> List[str]:
    ...
    # Aba (tenta por nome exato; se falhar, tenta match por upper)
    try:
    ...
    norm = []
    for r in rows:
        rr = {str(k).strip().upper(): r[k] for k in r}
        norm.append(rr)
...
if not sheet_ids:
    st.error("Nenhum arquivo ATIVO com URL/ID válido na aba ARQUIVOS da planilha-índice.")
    st.info("Verifique: compartilhamento do índice com a conta de serviço; coluna URL/LINK; coluna ATIVO marcada como S.")
    st.stop()
...
if not col_unid or not col_chassi or "__DATA__" not in df.columns or "VISTORIADOR" not in df.columns or "IS_REV" not in df.columns:
    st.error("Base consolidada não contém as colunas necessárias (UNIDADE, CHASSI, __DATA__, VISTORIADOR, IS_REV).")
    st.stop()
...
cards = [
    ...
    ("Revistorias",         f"{revistorias_total:,}".replace(",", ".")),
    ("% Revistorias",       f"{pct_rev:,.1f}%".replace(",", "X").replace(".", ",").replace("X", ".")),
]
st.markdown(
...
# =========================
# Resumo por Vistoriador (filtro FIXO/MÓVEL só aqui)
# =========================
...
# METAS: mês do filtro selecionado (ref_year/ref_month)
ref_ym = f"{ref_year}-{ref_month:02d}"
metas_ref = (
    df_metas_all[df_metas_all["__YM__"] == ref_ym].copy()
...
# Normaliza TIPO + filtro apenas nesta tabela
...
sel_tipos = st.multiselect(
    ...
    key="resumo_tipo_filter_starcheck"
)
...
if fmt.empty or not cols_show_avail:
    ...
    st.download_button("Baixar resumo (CSV)", data=csv, file_name="resumo_vistoriador_starcheck.csv", mime="text/csv")
...
if len(datas_ok) == 0:
    ...
    def render_ranking(df_sub, titulo):
        ...
        top_fmt = pd.DataFrame({
            ...
            "% Ating.": top["ATING_%"].map(chip_pct_row),
        })
        ...
        bot_fmt = pd.DataFrame({
            ...
            "% Ating.": bot["ATING_%"].map(chip_pct_row),
        })

        c1, c2 = st.columns(2)
        with c1:
            st.markdown(f"**TOP BOX — {mes_label}**")
            st.dataframe(top_fmt, use_container_width=True, hide_index=True)
        with c2:
            st.markdown(f"**BOTTOM BOX — {mes_label}**")
